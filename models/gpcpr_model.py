...
from models.similarity_head import ShallowSimilarityHead, DeepSimilarityHead, LogitsFusion
...
class GPCPR(nn.Module):
    ...
    def forward(self, support_x, support_y, query_x, query_y, text_emb=None, text_emb_diff=None):
        ...
        if self.use_attention:
            support_feat, _, support_shallow = self.getFeatures(support_x)
        else:
        ...
        if self.use_attention:
            query_feat, _, query_shallow = self.getFeatures(query_x)
        else:
    ...
    def getFeatures(self, x):
        ...
        :return: features with shape (B, C_out, L), shallow_features with shape (B, 64, L)
        """
        if self.use_attention:
            # 假设encoder返回包含x_shallow的结果
            feat_level1, feat_level2, xyz, x_shallow = self.encoder(x)
            feat_level3 = self.base_learner(feat_level2)
            att_feat = self.att_learner(feat_level2)
            if self.use_linear_proj:
                return self.conv_1(
                    torch.cat((feat_level1[0], feat_level1[1], feat_level1[2], att_feat, feat_level3),
                              dim=1)), xyz, x_shallow
            else:
                return torch.cat((feat_level1[0], feat_level1[1], feat_level1[2], att_feat, feat_level3),
                                 dim=1), xyz, x_shallow
        else:
            # 假设encoder返回包含x_shallow的结果
            feat_level1, feat_level2, x_shallow = self.encoder(x)
            feat_level3 = self.base_learner(feat_level2)
            map_feat = self.linear_mapper(feat_level2)
            return torch.cat((feat_level1, map_feat, feat_level3), dim=1), x_shallow
